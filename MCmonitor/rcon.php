...
?>
<?php	function	get_top($str) {
		$pos = strpos($str, "Total:");
		$i = 1;
		while (is_numeric($str[$pos + 9 + ++$i]));
		$ret = substr($str, $pos + 9, $i);
		return $ret;
	}
?>
<?php	function	get_ranking($str) {
		$test = explode('.', $str);
		$old = array(", ", "§b epi§r", "§r", " §6Type§c /balancetop 2 §6to read the next pag");
		$new   = array("</td><td>", "</td>", "", "");

		for ($i = 1; isset($test[$i]) && $i != 9; $i++) {
			$test[$i] = str_replace($old, $new, $test[$i]);
			$test[$i] = substr($test[$i], 1,(strlen($test[$i]) - 2));
			echo "<tr><td>" . $test[$i] . "</tr>";
		}
	}
		function	get_rankings($a) {
		$b = str_replace(array('§c', '§r', ':', ','), '', $a); // je supprime les caractères inutiles
		$c = explode('.', $b);                                 // j'explode() la chaine à partir de chaque point 
			// je supprime les clés vides -> array_filter() est un peu optionnel d'autant plus que seront supprimées les clés retournant uniquement '0' -> dans ce cas faudra tenir compte du if(count($e) === 2)
		foreach(array_filter($c) as $v) {         
		$d = strstr($v, '§b', true);            // je découpe $v par rapport à la postion de '§b'
		$e = array_slice(explode(' ', $d), -2); // j'explode $d à partir des espaces blancs et je recupère avec array_slice() les 2 dernières valeur -> une pour "clé" et une pour "valeur"
		// if(count($e) === 2) {                // je m'assure du nombre de clés
			$f[$e[0]] = $e[1];                  // je recupère le tout dans un tableau 
        // ou je crée mon html
			if (!empty($e[0]) && (strcmp($e[0], "Total") !== 0)) {
